su -
ssh -p 6022 izzydharma@localhosy
ssh -p 6022 izzydharma@localhost
poweroff
ssh -p 6022 izzydharma@localhost
ssh
...
ls -F
exit
scp -P 6022 .\file?.txt izzydharma@localhost:/tmp/
(((type nul 2> nul > file1.txt)>file2.txt)>file3.txt)>file4.txt;cmd /r dir/w
su -
git config --global user.email        '147742855+izzydharma@users.noreply.github.com'
git config --global user.name         'izzydharma'
...
git remote -v
git init
git remote add origin https://github.com/izzydharma/os242
git remote -v
git pull
...
if [ -d $DEMOS ] ; then   cd $DEMOS/;   git pull;   mkdir -vp $HOME/mydemo/;   cd $HOME/mydemo/;   /bin/cp -rfv $DEMOS/Week00/ .;   /bin/cp -rfv $DEMOS/$WEEK/  .;   cd $WEEK/;   ls -al; else   echo "ERROR: no $DEMOS directory!"; fi
bash a06-loop os242 izzydharma 2306256425
echo "ZCZC I_AM=$(whoami) HOST=$(hostname) NPROC=$(nproc) LANGUAGE=$LANGUAGE ARCH=$(arch) $(lscpu|grep -E 'Model name'|sed 's/ \+/ /g')"
...
mkdir -pv $HOME/git/
cd $HOME/git/
git clone git@github.com:izzydharma/os242.git
mkdir -pv $HOME/git/
cd $HOME/git/
...
git restore --staged mydemo/Week00/Makefile
git status
git add .gitfconfig
git status
gitt add .gitconfig
...
LANGUAGE="en_AU.UTF-8"
LC_ALL=""
LC_ALL="# File: /etc/default/locale (root)
LC_CTYPE="en_AU.UTF-8"
LANG="en_AU.UTF-8"
LANGUAGE="en_AU.UTF-8"
LC_ALL=""en_AU.UTF-8"
locale
setx LC_CTYPE "en-AU.UTF-8" /M
setx LANG "en-AU.UTF-8" /M
setx LANGUAGE "en-AU.UTF-8" /M
setx LC_ALL "" /M
locale
git pull
git status
...
DEMOS=$HOME/git/demos/Demos
if [ -d $DEMOS ] ; then   cd $DEMOS/;   git pull;   mkdir -vp $HOME/mydemo/;   cd $HOME/mydemo/;   /bin/cp -rfv $DEMOS/Week00/ .;   /bin/cp -rfv $DEMOS/$WEEK/  .;   cd $WEEK/;   ls -al; else   echo "ERROR: no $DEMOS directory!"; fi
bash a06-loop os242 cbkadal 1234567890
su -
ssh -T git@github.com
date
whoami
...
WEEK=Week02
DEMOS=$HOME/git/demos/Demos/
if [ -d $DEMOS ] ; then   cd $DEMOS;   git pull;   mkdir -vp $HOME/mydemo/;   cd $HOME/mydemo/;   cp -r $DEMOS/$WEEK/ .;   cd $WEEK/;   ls -al; else   echo "ERROR: no $DEMOS directory!"; fi
...
gpg --list-key --keyid-format short
gpg --list-keys
gpg --list-key A076EAD4A9CC88E9D83544DC28D12D804D20E4B5

gpg --list-keys 28D12D804D20E4B5
gpg --list-key --keyid-format short
gpg --list-key /4D20E4B5
gpg --list-key 4D20E4B5
gpg --list-key cbkadal@localhost
gpg --list-key izzydharma@localhost
gpg --list-key izzydharma
cd /tmp/
...
cd $HOME/git/os242/TXT/
rm -f mypubkey.txt
gpg --output mypubkey.txt --armor --export izzydharma@localhost
ls -al
cd
...
if [ -d $TARDIR/ ] ; then   cd $TARDIR/;   git pull;   mkdir -pv $HOME/mywork/;   cd $HOME/mywork/;   cp -v $TARDIR/$TARBALL .;   ls -al; else   echo "ERROR: no $TARDIR directory!"; fi
